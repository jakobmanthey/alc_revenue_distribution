...
#ESA
esa_path <- file.path("../../ESA", "ESA_Alc_Share_240624.xlsx")
...
getraenketypen <- c('bier','wein', 'sprit', 'misch')
...
priceperunit <- temp_df %>%
  ...
  mutate(unit = ifelse(grepl("bier", Getraenketyp), "0,33l",
                   ifelse(grepl("^wein", Getraenketyp), "0,75l",
                      ifelse(grepl("sprit", Getraenketyp), "0,7l", "0"))))
...
terzile_einkommen_grenzen_EUSILC <- c(-Inf, 1355, 2572, Inf)
...
Zunächst rechne ich die gekauften Mengen in Standarddrinks um, um die Getränketypen vergleichbar zu machen.
...
#note: "Sprit Anderes" examples are Korn, Rum, Wodka, -> used the catergory "Schnapps" from Table 1, Kuitunen-Paul et al. 2017
...
```{r n mixdrink}
sum(evs$mischgetraenke_menge > 0) # nur 62 Haushalte haben Mischgetränke gekauft
```
...
length(unique(evs_adults$ID[evs_adults$gesamtmenge_stdd_w == 0])) # n = 2850 haushalte
...
aggregated_spendings <- evs_strata_trinkgruppe %>%
  ...
  summarize(
    ...
    misch_n = sum(!is.na(Mischgetraenke_Wert_pro_stdd_w)))
