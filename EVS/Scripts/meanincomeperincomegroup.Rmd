...
Wieso so hoch? Wie genau zu interpretieren? 
Vorerst ignorieren
```{r weights bund}
str(evs_below65$weight_bund)
#weight_bund is a character, convert to numeric
...
evs_below65$gewichtungsfaktor_grouped <- cut(evs_below65$gewichtungsfaktor, #neue Spalte mit Einkommensgruppen bilden
                            ...
                            labels = c("1", "1.3-1.5", "1.6-2", "2.1-4.1"), include.lowest = TRUE, right = FALSE)
```
...
income_perincomegroupweight <- evs_below65 %>%
  ...
  summarise(mean_haushaltsnettoeinkommen_month = mean(haushaltsnettoeinkommen_month, na.rm = TRUE),
...
#export data as csv
if (exportdata) {
  write.csv(income_perincomegroupweight, file = paste0(output_path, "income_perincomegroupweight_v1.csv"), row.names = FALSE)
}
```